...
The hook auto-detects Hermes' `pre_llm_call` payload and fires on the first turn whose conversation history contains a Hermes compaction handoff (`_compressed_summary` row metadata, or the `[CONTEXT COMPACTION — REFERENCE ONLY]…` summary, the no-user-turn continuation marker, the `## Historical Task Snapshot` fallback, or a merged summary as byte-pinned fallbacks) — the live user message carried by the immediate post-compaction turn is included, because Hermes compacts at turn start; a per-session dedupe (`session_id` + summary identity, 12 h TTL) keeps the persisting summary row from re-triggering later turns. Ordinary turns print a `{}` no-op JSON line. On injection it answers with a single `{"context": "<markdown breadcrumb block>"}` stdout line, the only channel Hermes parses from shell-hook stdout. It resolves the repository from the payload's `cwd`. Approve the `(pre_llm_call, node)` hook pair on first use (or set `hooks_auto_accept: true` / run `hermes --accept-hooks`), then restart Hermes. See [`harnesses/hermes/README.md`](../harnesses/hermes/README.md) for the complete contract.
